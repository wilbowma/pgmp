...
We implement the function @nonbreaking{@racket[profile-query]} by simply
querying this map.
The function @racket[load-profile] updates this map from a file.
To write this map to a file, the function @racket[store-profile] first
retrieves the profile information from the profiler and computes the
profile weights for each source object.
...
new syntax object.
A separate library exists that provides a more extensive API for
manipulating source information.
We use this library to implement @racket[make-profile-point] and
@nonbreaking{@racket[annotate-expr]} in essentially the same way as in Chez Scheme.
...
Doing so ensures that profile information remains consistent at both the
source-level and the block-level.
First, we compile while instrumenting the code to profile source
expressions.
...
After running this instrumented program, we get the profile weights for
the basic blocks generated from the optimized source program.
These block-level profile weights will not be invalidated as long as we
continue to optimize using the source-level profile weights.
That is, the high-level code generated by the meta-program
optimizations, e.g., @Figure-ref{sample-expansion}, will remain stable
as long as we continue to optimize using the source profile weights.
Because the generated high-level code remains stable, the generated
low-level code also remains stable.
Third (the final compilation), we recompile using both the profile
weights for the source expressions and for the basic blocks to do both
profile-guided meta-programming and @nonbreaking{low-level PGOs.}
