...
This case study shows that our design is expressive enough to implement
this well-known PGO.
...
information.
The @racket[exclusive-cond] expression is generated by rewriting each
@racket[case] clause into an @racket[exclusive-cond] clause.
...
programmer to encode their knowledge, e.g., that the branches of this
conditional are mutually exclusive, in order to take advantage of
optimizations that would have otherwise been impossible.
...
by profile weight and generates a regular
@racket[cond]. Since each @racket[exclusive-cond] clause is also a
@racket[cond] clause, the clauses do not need to be transformed.
...
@racket[else] clause that is never reordered, is 31 lines.
@Figure-ref{case-expansion} shows the code generated from the example @racket[case]
expression in @Figure-ref{case-example}
...
This case study demonstrates that our design is expressive enough to
implement a well-known PGO via meta-programming.
It also illustrates some important features of our design.
...
programmers can encode domain-specific knowledge into @nonbreaking{the
program.}
